...
<br>
Spectral-temporal characteristics and intensity of the STED beam are not the only parameters affecting the resolution of a STED microscopy image. The spatial resolution of a STED image depends also on the microscope alignment (e.g., temporal and spatial overlap of STED and excitation beams), the sample- and specimen-induced aberrations, and the sample conditions (e.g., fluorophores brightness and photostability). The optimisation of these conditions is a tedious task which need time consuming calibration experiments. Our group developed a fully automatic method, based on the Fourier ring correlation (FRC) approach, which extracts directly from the image of the sample-of-interest its resolution. This information can be used to optimise the microscope status, the imaging conditions and the sample preparation. Furthermore, the FRC analysis can be used to simplify and automatise image restoration, e.g., image deconvolution.
<br>
